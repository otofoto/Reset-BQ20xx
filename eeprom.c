/* Written by Chris <chris@hedonism.cx>                                     */
/* distributed with the lm_sensors package, in the prog/eepromer/ directory */
/* This version was taken from lm_sensors 2.6.4 (still the same in 2.7.0)   */
/* and was updated to support 24c01 and 24c02 (MAX_BYTES=8) by              */
/* Frank Rysanek <Frantisek.Rysanek@post.cz>                                */
/* This patch should appear in the next version of lm_sensors after 2.7.0.  */
...
int main(int argc, char **argv){
    ...
    while((i=getopt(argc,argv,"d:a:p:t:wyf:h"))>=0){
	switch(i){
	case 'h':
	    fprintf(stderr,"%s [-d dev] [-a adr] [-p pgs] [-t type] [-w] [-y] [-f file]\n",argv[0]);
	...
	case 'a':
	    ...
	    break;
	case 't':
	    if (!(strncmp(optarg,"24c01",5) && strncmp(optarg,"24C01",5))){
               if (pgs_specified == 0) pages = 1;
               bytes_per_page = 128;
               fprintf(stderr,"chip type                     : 24C01");
               fprintf(stderr," (=> Setting bytes_per_page to 128.)\n");
            }
            else if (!(strncmp(optarg,"24c02",5) && strncmp(optarg,"24C02",5))){
               if (pgs_specified == 0) pages = 1;
               fprintf(stderr,"chip type                     : 24C02\n");
            }
            else if (!(strncmp(optarg,"24c04",5) && strncmp(optarg,"24C04",5))){
               if (pgs_specified == 0) pages = 2;
               fprintf(stderr,"chip type                     : 24C04\n");
            }
            else if (!(strncmp(optarg,"24c08",5) && strncmp(optarg,"24C08",5))){
               if (pgs_specified == 0) pages = 4;
               fprintf(stderr,"chip type                     : 24C08\n");
            }
            else if (!(strncmp(optarg,"24c16",5) && strncmp(optarg,"24C16",5))){
               if (pgs_specified == 0) pages = 8;
               fprintf(stderr,"chip type                     : 24C16\n");
            }
            else {
		fprintf(stderr,"Cannot parse '%s' as chip type (example: 24C02)\n",
			optarg);
                exit(1);
            }
